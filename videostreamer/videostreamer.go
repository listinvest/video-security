...
//Run cvcvcv
func Run(rw http.ResponseWriter, r *http.Request, rtsp string, verbose bool) {
	C.vs_setup()

	// Clients provide encoder info about themselves when they start up.
	clientChan := make(chan *Client)

	go encoder("rtsp", rtsp, verbose, clientChan)
...
func encoder(inputFormat, inputURL string, verbose bool, clientChan <-chan *Client) {
	clients := []*Client{}
	var input *Input
